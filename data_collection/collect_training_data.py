...
class CollectTrainingData(object):
    ...
    def collect(self):
        ...
        try:
            ...
            if not os.path.exists(os.path.join(data_dir,training_dir)):
                os.makedirs(os.path.join(data_dir,training_dir))
            if os.path.isfile(training_dir + "/" + file_name):
                training_data = list(np.load(os.path.join(data_dir,training_dir,file_name), allow_pickle=True))
            else:
                training_data = []
            ...
            while 1:
                ...
                if len(training_data) % training_pack == 0:
                    count += 1
                    totalimg += 1
                    np.save(os.path.join(data_dir,training_dir, f"{file_name}-{int(totalimg / 6)}"), training_data)
